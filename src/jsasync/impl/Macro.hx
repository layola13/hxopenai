...
class Macro {
	...
	static function transformAwait(e:Expr) {
		return switch e.expr {
			case EMeta({name: "await"}, innerExpr):
				macro jsasync.JSAsyncTools.jsawait($innerExpr);
				
			case EFor(it, body):
				switch it.expr {
					case EMeta({name: "await"}, {expr: EBinop(OpIn, {expr: EConst(CIdent(name))}, asyncIter)}):
						var itemIdent = Context.parse(name, e.pos);
						macro {
							var __iterator = jsasync.JSAsyncTools.getAsyncIterator($asyncIter);
							while(true) {
								var __next = jsasync.JSAsyncTools.jsawait(__iterator.next());
								if(__next.done) break;
								var $name = __next.value;
								$body;
							}
						}
					default: e.map(transformAwait);
				}
    ...
    static function transformForAwait(iterable:Expr, body:Expr) {
        return macro {
            var iter = jsasync.JSAsyncTools.getAsyncIterator($iterable);
            while(true) {
                var next = jsasync.JSAsyncTools.jsawait(iter.next());
                if(next.done) break;
